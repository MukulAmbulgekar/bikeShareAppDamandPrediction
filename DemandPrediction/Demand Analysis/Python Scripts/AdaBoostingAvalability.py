if __name__ == '__main__':
    ...
    print '\nRunning Random Forest Regressor for avalability...\n'
    
    for i_station in uniqueStations:
        print(i_station)
        trainPath = '../DataStatus/'+str(i_station)+'_BikeStation_Status_Train.json'
        train = pd.read_json(trainPath)
        testPath = '../DataStatus/'+str(i_station)+'_BikeStation_Status_Test.json'
        ...
        clf = ensemble.AdaBoostRegressor(n_estimators=200)         
        ...
        #print(df)
        df.reset_index().to_json('../Prediction/AdaBoostingStatus/'+str(i_station)+'_Prediction_Status_AB.json',orient='records')
